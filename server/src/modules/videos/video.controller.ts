...
export async function uploadVideoHandler(req: Request, res: Response){
    ...
    // to take a chunk of the file and store it in memory, to handle memory efficiently
...
export async function updateVideoHandler(req: Request<UpdateVideoParams, {}, UpdateVideoBody>, res: Response){
    ...
    if(String(video.owner) !== String(userId)){
        return res.status(StatusCodes.UNAUTHORIZED).send('You are not the owner of this video')
    }
...
export async function streamVideoHandler(req: Request, res: Response){
    ...
    if(!range){
        return res.status(StatusCodes.BAD_REQUEST).send('Range header is missing')
    }
